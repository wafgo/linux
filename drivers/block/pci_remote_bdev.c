...
static struct pci_bio_driver_ring *get_driver_ring(struct pci_remote_block_device *rdev)
{
    return (struct pci_bio_driver_ring *)((u64)rdev->descr_ring + rdev->drv_offset);
}

static struct pci_bio_device_ring *get_device_ring(struct pci_remote_block_device *rdev)
{
    return (struct pci_bio_device_ring *)((u64)rdev->descr_ring + rdev->dev_offset);
}
...
/* 	    dev_info(dev, "Dev Ring has some completed entries. Internal Index: %i, Device Index: %i. Descriptor sector 0x%llX, addr: 0x%llX, opf: %s, len: 0x%x, offset: 0x%x\n", rdev->dev_idx, dev_ring->idx, desc->s_sector, desc->addr, (desc->opf == WRITE) ? "WRITE" : "READ", desc->len, desc->offset); */
/* 	    print_hex_dump(KERN_WARNING, "raw: ", DUMP_PREFIX_OFFSET, 16, 16, buf, 0x50, true); */
...
/* 	/\*     dev_err(&rb_req->rdev->pdev->dev, "%i: Req Sector: 0x%llX, len: 0x%x \n", i, (u64)blk_rq_pos(rq), blk_rq_bytes(rq)); *\/ */
/* 	/\*     dev_err(&rb_req->rdev->pdev->dev, "%i: PageAddress: 0x%llX, offset: 0x%x, len: 0x%x \n", i, (u64)page_address(bv.bv_page), bv.bv_offset, bv.bv_len); *\/ */
/* 	/\*     ++i; *\/ */
/* 	/\* } *\/ */
...
/* 	else */
/* 	    queue_work(deferred_bio_add_workqueue, work); */
/* 	/\* rb_req->num_bios = 0; *\/ */
...
/* 	dev_err(&rbd->rdev->pdev->dev, "%s called\n", __FUNCTION__);  */
...
static int get_and_mark_free_descriptor(struct pci_remote_block_device *rdev)
{
	...
	if (!(de->flags & PBI_EPF_BIO_F_USED)) {
	    dev_info(dev, "Found free descriptor at idx %i\n", i);
				     ...
				     const struct blk_mq_queue_data *bd)
{
	...
	rb_req->drv_ring_idx = descr_idx;
	dtu = &rdev->descr_ring[descr_idx];
	rb_req->pg = alloc_pages(GFP_KERNEL, get_order(blk_rq_bytes(bd->rq)));
	if (!rb_req->pg) {
		dev_err(dev, "%s: OOM\n", __FUNCTION__);
		return BLK_STS_RESOURCE;
	}
	
	dma_addr = dma_map_single(dev, page_address(rb_req->pg), blk_rq_bytes(bd->rq), DMA_FROM_DEVICE);
	dtu->addr = dma_addr;
	...
	dtu->tag = (u64)rb_req;
	drv_ring->ring[drv_idx].index = descr_idx;
	    
	drv_ring->idx = (drv_idx + 1) % NUM_DESRIPTORS;
	dev_info(dev, "%i (DIR: %s): Adding descriptor index %i. Addr: 0x%llX, sector: 0x%llX, len: 0x%x, offset: 0x%x\n", i, (rq_data_dir(bd->rq) == WRITE) ? "WRITE" : "READ", descr_idx, dma_addr, dtu->s_sector, dtu->len, dtu->offset);

	blk_mq_start_request(bd->rq);
	wake_up_process(rdev->dp_thr);
	/* rdev->base->command = COMMAND_ADD_DRV_ENTRY; */
	...
	/*     dev_info(dev, "%i: Req Sector: 0x%llX, len: 0x%x \n", i, (u64)blk_rq_pos(bd->rq), blk_rq_bytes(bd->rq)); */
	/*     dev_info(dev, "%i: PageAddress: 0x%llX, offset: 0x%x, len: 0x%x \n", i, (u64)page_address(bv.bv_page), bv.bv_offset, bv.bv_len); */
	...
	/*     dtu = &rdev->descr_ring[descr_idx]; */
	/*     dma_addr = dma_map_single(dev, page_address(bv.bv_page), bv.bv_len + bv.bv_offset, DMA_FROM_DEVICE); */
	/*     dtu->addr = dma_addr; */
	/*     dtu->len = bv.bv_len; */
	/*     dtu->offset = bv.bv_offset; */
	/*     dtu->opf = iter.bio->bi_opf; */
	    
	/*     dtu->s_sector = (i == 0) ? blk_rq_pos(bd->rq) : -1; */
	/*     drv_ring->ring[drv_idx].index = descr_idx; */
	    
	/*     if (rq_iter_last(bv, iter)) */
	/* 	drv_ring->ring[drv_idx].flags |= PBI_EPF_BIO_F_LAST; */
	/*     else */
	/* 	drv_ring->ring[drv_idx].flags &= ~PBI_EPF_BIO_F_LAST; */
	    
	/*     drv_idx = (drv_idx + 1) % NUM_DESRIPTORS; */
	/*     dev_info(dev, "%i: Adding descriptor index %i. Addr: 0x%llX, sector: 0x%llX, len: 0x%x, offset: 0x%x\n", i, descr_idx, dma_addr, dtu->len, dtu->offset); */
	/*     ++i; */
	/* } */
...
static int pci_rbd_open(struct block_device *bdev, fmode_t mode)
{
	...
	dev_err(&rdev->pdev->dev,"%s called\n", __FUNCTION__);
	return 0;
}
...
{
    struct pci_remote_block_device *rdev = (struct pci_remote_block_device *)cookie;
    struct device *dev = &rdev->pdev->dev;
    struct pci_bio_device_ring *dev_ring = get_device_ring(rdev);
    struct req_iterator iter;
    struct bio_vec bv;
	...
	while (dev_ring->idx != rdev->dev_idx) {
	    int descr_idx = dev_ring->ring[rdev->dev_idx].index;
	    struct pci_epf_bio_descr *desc = &rdev->descr_ring[descr_idx];
	    struct pci_remote_bd_request *rb_req = (struct pci_remote_bd_request *)desc->tag;
	    struct request *rq = blk_mq_rq_from_pdu(rb_req);
	    void *buf = kmap(rb_req->pg);
	    int i = 0;
		...
		dev_info(dev, "%i: bv.len = 0x%x, bv.offset = 0x%x, page: 0x%llX", i, bv.bv_len, bv.bv_offset, page_address(bv.bv_page));
		buf += bv.bv_len;
	    }
			       ...
			       const struct pci_device_id *ent)
{
	...
	rdev->dp_thr = kthread_create(pci_remote_bd_dispatch, rdev, "pci_rbd_thread");
	...
	rdev->descr_size =  ALIGN(NUM_DESRIPTORS * sizeof(*rdev->descr_ring), sizeof(u64)) + ALIGN(sizeof(struct pci_bio_driver_ring) + (NUM_DESRIPTORS * sizeof(struct pci_bio_driver_ring_entry)), sizeof(u64)) + ALIGN(sizeof(struct pci_bio_device_ring) + (NUM_DESRIPTORS * sizeof(struct pci_bio_device_ring_entry)), sizeof(u64)) + alignment; 
	...
	orig_phys_addr = dma_map_single(dev, rdev->descr_ring, rdev->descr_size + alignment, DMA_FROM_DEVICE);
	...
	if (!IS_ALIGNED(orig_phys_addr, alignment)) {
		...
		rdev->descr_ring = (struct pci_epf_bio_descr __iomem *)((u64)rdev->descr_ring + offset);
	} else {
		phys_addr = orig_phys_addr;
	}

	rdev->drv_offset = ALIGN(NUM_DESRIPTORS * sizeof(*rdev->descr_ring), sizeof(u64));
	rdev->dev_offset = rdev->drv_offset + ALIGN(sizeof(struct pci_bio_driver_ring) + (NUM_DESRIPTORS * sizeof(u16)), sizeof(u64));
	
	rdev->base->drv_offset = rdev->drv_offset;
	rdev->base->dev_offset = rdev->dev_offset;

	dev_info(dev, "-----> Setting queue addr @ 0x%llX -> 0x%llx. Queue Size for %i descriptors = %i. Offset = %i\n", rdev->descr_ring, phys_addr, NUM_DESRIPTORS, rdev->descr_size, offset);
	...
	dev_err(dev, "-----> First Timeout \n");
	timeout = jiffies + 500;
        while(!time_after(jiffies, timeout)) {

	}
	dev_err(dev, "-----> Second Timeout \n");
	for (i = 0; i < rdev->num_irqs; i++) {
		err = devm_request_irq(dev, pci_irq_vector(pdev, i),
				       pci_rbd_irqhandler,
				       IRQF_SHARED, "RBD IRQ", rdev);
		if (err) {
	...
	for (i = 0; i < 16; ++i) {
		...
		dev_info(dev, "%i: Lets look what was written: addr: 0x%llX, s_sector: 0x%llX, s_offset = 0x%x, num_sectors = 0x%x", i, or->addr, or->s_sector, or->offset, or->len);
	...
	snprintf(rdev->gd->disk_name, 32, "pci_rbd-%s", &rdev->base->dev_name[5]);
